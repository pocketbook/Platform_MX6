...
#define DBG(x...) do { printk ("[%s] ",__func__); printk (x); printk ("\n"); } while (0)
...
int rt5640_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	int sclk_src;
	int reg63, reg64;
	DBG("jack = %i; bias=%i",jack_insert,codec->dapm.bias_level);
	if(jack_insert) {
...
static void rt5640_mute_int(struct snd_soc_codec *codec);

#if USE_ONEBIT_DEPOP
void hp_amp_power(struct snd_soc_codec *codec, int on)
{
	static int hp_amp_power_count;
	DBG("one bit hp_amp_power on=%d hp_amp_power_count=%d",on,hp_amp_power_count);
...
//				RT5640_HP_CP_PU | RT5640_HP_SG_DIS | RT5640_HP_CB_PU);
			...
			snd_soc_update_bits(codec, RT5640_DEPOP_M1,
					RT5640_HP_CP_MASK | RT5640_HP_CB_MASK | RT5640_HP_SG_EN,
					RT5640_HP_CP_PU | RT5640_HP_CB_PU | RT5640_HP_SG_EN);
...
//				RT5640_PWR_FV1 | RT5640_PWR_FV2 );
			...
			msleep(50);
//			snd_soc_update_bits(codec, RT5640_DEPOP_M1,
//					RT5640_HP_CO_MASK | RT5640_RSTP_MASK | RT5640_RSTN_MASK | RT5640_HP_CD_PD_MASK ,
//					RT5640_HP_CO_EN | RT5640_RSTP_EN | RT5640_RSTN_EN | RT5640_HP_CD_PD_EN);
...
//		if(hp_amp_power_count <= 0) {
		snd_soc_update_bits(codec, RT5640_DEPOP_M1,
				RT5640_HP_CP_MASK | RT5640_HP_SG_MASK,
				RT5640_HP_CP_PD | RT5640_HP_SG_DIS);
...
static void rt5640_unmute_int(struct snd_soc_codec *codec)
{
...
//	snd_soc_update_bits(codec, RT5640_DEPOP_M1,
//			RT5640_SMT_TRIG_MASK | RT5640_HP_L_SMT_MASK | RT5640_HP_R_SMT_MASK,
//			RT5640_SMT_TRIG_DIS | RT5640_HP_L_SMT_DIS | RT5640_HP_R_SMT_DIS);
...
#endif
		//rt5640_unmute_int(codec);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		//rt5640_mute_int(codec);
		break;
			...
			unsigned int freq_in, unsigned int freq_out)
{
	...
	DBG("%i; %i; %i; %i",pll_id,source,freq_in,freq_out);
...
static ssize_t hp_mute_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt5640_priv *rt5640 = i2c_get_clientdata(client);
	...
	val = codec->hw_read(codec, RT5640_HP_VOL);
	cnt = sprintf(buf,"%s\n",(val & (RT5640_L_MUTE | RT5640_R_MUTE)) ? "1" : "0");

	return cnt;
}

static ssize_t hp_mute_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt5640_priv *rt5640 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = rt5640->codec;
	...
	DBG("read \"%s\" buf; count = %i",buf,count);
...
static DEVICE_ATTR(hp_mute, 0666, hp_mute_show, hp_mute_store);
...
static int rt5640_probe(struct snd_soc_codec *codec)
{
	...
	ret = device_create_file(codec->dev, &dev_attr_hp_mute);
	if (ret != 0) {
		dev_err(codec->dev,
			"Failed to create hp_mute sysfs files: %d\n", ret);
		return ret;
	}
