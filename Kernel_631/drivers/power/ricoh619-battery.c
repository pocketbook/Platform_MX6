...
extern void set_led_status (int status);
extern void led_green(int);
extern void set_charging_led(int status);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		else if(SDP_CHARGER==giRICOH619_DCIN) {
			val->intval = 2;
		}
		else {
			val->intval = 1;
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		//stick for charging led
		set_charging_led(val->intval);
...
#ifdef false //ENABLE_FUEL_GAUGE_FUNCTION
		if (info->soca->ready_fg)
