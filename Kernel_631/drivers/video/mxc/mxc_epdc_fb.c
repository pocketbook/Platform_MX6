...
#undef READ_VCOM_FROM_FILE

//#define DDD(s...) printk("<7>EPDC -- " s)
#define DDD(s...)

#include "eink_processing2.h"

// from mxc_wf_parser.c
...
#define ERR(x...)  printk("<1>" x)
#define INF(x...)  printk("<1>" x)
//#define DBG(x...)  printk("<7>" x)
#define DBG(x...) 

#define WF_MAX_SIZE (6*1024*1024)
...
static const char *waveform_filename = "/dev/mmcblk0";
static const int waveform_name_offset = 14334 * 512;
static const int waveform_offset = 32768 * 512;

//static const char *waveform_filename = "/boot/default.wbf";
//static const int waveform_name_offset = 0;
//static const int waveform_offset = 0;

static const char *vcom_filename = "/dev/mmcblk0";
static const int vcom_offset = 1289 * 512;
static int vcom_value = 0;

static char waveform_name[128];

static unsigned long default_bpp = 8;

static int logo_in_progress = 0;
static int user_update_issued = 0;
static int shadow_fb_info = 0;

unsigned char gbModeVersion=0 ;
unsigned char gbWFM_REV ;
unsigned char gbFPL_Platform ;
unsigned char gbRegal = 0;
...
struct mxc_epdc_fb_data {
	...
	struct delayed_work v3p3_off_work;
...
struct waveform_data_header {
	unsigned int wi0;		// 0x00
	unsigned int wi1;		// 0x04
	unsigned int wi2;		// 0x08
	unsigned int wi3;		// 0x0c
	unsigned int wi4;		// 0x10
	unsigned int wi5;		// 0x14
	unsigned int wi6;		// 0x18
	unsigned int xwia:24;		// 0x1c
	unsigned int cs1:8;		// 0x1f
	unsigned int wmta:24;		// 0x20
	unsigned int fvsn:8;		// 0x23
	unsigned int luts:8;		// 0x24
	unsigned int mc:8;		// 0x25
	unsigned int trc:8;		// 0x26
	unsigned int awv:8;  	// 0x27
	unsigned int eb:8;		// 0x28
	unsigned int sb:8;		// 0x29
	unsigned int reserved0_1:8;	// 0x2a
	unsigned int reserved0_2:8;	// 0x2b
	unsigned int reserved0_3:8;	// 0x2c
	unsigned int reserved0_4:8;	// 0x2d
	unsigned int reserved0_5:8;	// 0x2e
	unsigned int cs2:8;		// 0x2f
};
...
struct mxc_epdc_fb_data *g_fb_data, *fb_data_shadow;
			    ...
			    struct pxp_config_data *pxp_conf)
{
	...
	INF("drect left 0x%x right 0x%x width 0x%x height 0x%x",
		pxp_conf->proc_data.drect.left, pxp_conf->proc_data.drect.top,
		pxp_conf->proc_data.drect.width,
		pxp_conf->proc_data.drect.height);
	INF("srect left 0x%x right 0x%x width 0x%x height 0x%x",
		pxp_conf->proc_data.srect.left, pxp_conf->proc_data.srect.top,
		pxp_conf->proc_data.srect.width,
		pxp_conf->proc_data.srect.height);
	INF("Scaling en 0x%x", pxp_conf->proc_data.scaling);
	INF("HFlip en 0x%x", pxp_conf->proc_data.hflip);
	INF("VFlip en 0x%x", pxp_conf->proc_data.vflip);
	INF("Rotation 0x%x", pxp_conf->proc_data.rotate);
	INF("BG Color 0x%x", pxp_conf->proc_data.bgcolor);
}
...
{
	INF(	"X = %d, Y = %d, Width = %d, Height = %d, WaveMode = %d, "
...
static void dump_collision_list(struct mxc_epdc_fb_data *fb_data)
{
	...
	list_for_each_entry(plist, &fb_data->upd_buf_collision_list, list) {
		INF("Virt Addr = 0x%x, Phys Addr = 0x%x ",
...
static void dump_all_updates(struct mxc_epdc_fb_data *fb_data)
{
	...
	INF("Current update being processed:\n");
	if (fb_data->cur_update == NULL)
		INF("No current update\n");
	else
	...
	u32 waveform_tempRange)
{
	...
	if (fb_data->waveform_vcd_buffer) {
		...
		if (mxc_epdc_fb_fetch_vc_data( fb_data->waveform_vcd_buffer, waveform_mode, waveform_tempRange, fb_data->waveform_mc, fb_data->waveform_trc, (unsigned char *) vcd) < 0)
			ERR(" *** Extra Waveform Data checksum error ***\n");
		else {
			DBG("VCData ver.0x%04x : v1=%04x,v2=%04x,v3=%04x,v4=%04x,v5=%04x,v6=%04x,v7=%02x\n",
...
static void epdc_submit_update(u32 lut_num, u32 waveform_mode, u32 update_mode,
			       bool use_dry_run, bool update_pause, bool use_test_mode, u32 np_val)
{
	u32 reg_val = 0;

	DBG("epdc_submit_update LUT=%d WF=%d MODE=%d\n", lut_num, waveform_mode, update_mode);
...
static void epdc_init_settings(struct mxc_epdc_fb_data *fb_data)
{
...
#if 1
	{
		...
		INF("HT(L%d,R%d,HSYNC%d) VT(U%d,L%d,VSYNC%d)\n",
			vmode->left_margin,vmode->right_margin,vmode->hsync_len,
			vmode->upper_margin,vmode->lower_margin,vmode->vsync_len);
...
static void v3p3_off_work_func(struct work_struct *work)
{
	...
	tps65185_v3p3_off();
	fb_data->v3p3_on = 0;
	fb_data->powering_down = 0;
}

static void vcom_set_voltage(struct mxc_epdc_fb_data *fb_data, int vcom_value, int vcomoffset)
{
	tps65185_vcom_set((-vcom_value) + vcomoffset, 0);
	INF("VCOM=-%d(%d)\n", vcom_value, vcomoffset);
}

static void epdc_powerup(struct mxc_epdc_fb_data *fb_data, int vcomoffset)
{
	...
	cancel_delayed_work_sync(&fb_data->v3p3_off_work);
	...
	ret = fb_data->v3p3_on ? 0 : tps65185_v3p3_on();
	fb_data->v3p3_on = 1;
	if (ret) {
		ERR("Unable to enable V3P3, err = 0x%x\n", ret);
		mutex_unlock(&fb_data->power_mutex);
		return;
	}
  ...
  // TODO: read VCOM from vcd data


        iMode = TPS65185_MODE_ACTIVE;
        tps65185_chg_mode(&iMode, 1);

	vcom_set_voltage(fb_data, vcom_value, vcomoffset);
	fb_data->vcom_on = 1;
...
static void epdc_powerdown(struct mxc_epdc_fb_data *fb_data, bool sync3v3)
{
	unsigned long iMode;
	...
	if (sync3v3) {
		mdelay(50);
		if (fb_data->v3p3_on) tps65185_v3p3_off();
		mdelay(10);
		fb_data->v3p3_on = 0;
	} else {
		schedule_delayed_work(&fb_data->v3p3_off_work,
...
static void epdc_init_sequence(struct mxc_epdc_fb_data *fb_data)
{
	...
	epdc_powerup(fb_data, 0);
	draw_mode0(fb_data);
	/* Force power down event */
	fb_data->powering_down = 1;
	epdc_powerdown(fb_data, true);
...
static int mxc_epdc_fb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	if (remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff,
		...
		INF("mmap remap_pfn_range failed\n");
		return -ENOBUFS;
	}
 ...
 *
 */
static int mxc_epdc_fb_set_par_really(struct fb_info *info)
{
	struct mxc_epdc_fb_data *fb_data = (struct mxc_epdc_fb_data *)info;
	...
	printk("mxc_epdc_fb_set_par_really: rotate=%d\n", screeninfo->rotate);
	...
	if (!fb_data->hw_ready) {
		...
		*/
		if (fb_data->cur_mode &&
			...
			if (!found_match) {
				ERR("Failed to match requested video mode\n");
				return EINVAL;
			}
		}
		...
		if (ret) {
			ERR("Failed to load panel waveform data\n");
			return ret;
		}
	}
...
{
	if (logo_in_progress) {
		printk("mxc_epdc_fb_set_par: copying shadow info, rotate=%d\n", info->var.rotate);
		memcpy(fb_data_shadow, info, sizeof(struct fb_info));
		shadow_fb_info = 1;
	} else {
		shadow_fb_info = 0;
		mxc_epdc_fb_set_par_really(info);
	}
	return 0;
}
...
static int mxc_epdc_fb_get_temp_index(struct mxc_epdc_fb_data *fb_data, int temp)
{
	...
	if (fb_data->trt_entries == 0) {
		ERR("No TRT exists...using default temp index\n");
	...
	if (index < 0) {
		ERR("No TRT index match...using default temp index\n");
		return DEFAULT_TEMP_INDEX;
	}

	DBG("Using temperature index %d\n", index);
...
int mxc_epdc_fb_read_temperature(struct mxc_epdc_fb_data *fb_data)
{
	...
	{
		if (tps65185_get_temperature(&temperature) == 0) {
			DBG("auto temperature reading = %d\n", temperature );
			fb_data->last_time_temp_auto_update = now;
			fb_data->temp_index = mxc_epdc_fb_get_temp_index(fb_data, temperature);
		} else {
			ERR("tps65185_get_temperature: read error\n");
		}
	}
	return 0;
...
EXPORT_SYMBOL(mxc_epdc_fb_set_temp_auto_update_period);
...
int mxc_epdc_fb_set_upd_scheme(u32 upd_scheme, struct fb_info *info)
{
	...
	DBG("Setting optimization level to %d\n", upd_scheme);
	...
	if ((upd_scheme == UPDATE_SCHEME_SNAPSHOT)
		...
		ERR("Invalid update scheme specified.\n");
		return -EINVAL;
	}
...
DDD("-- epdc_process_update %d,%d,%d,%d\n", 
	upd_desc_list->upd_data.update_region.left, 
	upd_desc_list->upd_data.update_region.top,
	upd_desc_list->upd_data.update_region.width,
	upd_desc_list->upd_data.update_region.height);
	...
	if (((((width_unaligned || height_unaligned || input_unaligned) &&
		...
		DDD("Copying update before processing.\n");
	...
	ret = pxp_process_update(fb_data, src_width, src_height,
		&pxp_upd_region);
	if (ret) {
		ERR("Unable to submit PxP update task.\n");
		mutex_unlock(&fb_data->pxp_mutex);
		return ret;
	}

	printk("waveform_mode=%d temp_index=%d\n", upd_desc_list->upd_data.waveform_mode, fb_data->temp_index);
	wfmode = upd_desc_list->upd_data.waveform_mode;

	/* If needed, enable EPDC HW while ePxP is processing */
	if ((fb_data->power_state == POWER_STATE_OFF)
		|| fb_data->powering_down) {
		epdc_powerup(fb_data, 0 /* TODO: voff */);
	}

	/* This is a blocking call, so upon return PxP tx should be done */
	ret = pxp_complete_update(fb_data, &hist_stat);
	if (ret) {
		ERR("Unable to complete PxP update task.\n");
		mutex_unlock(&fb_data->pxp_mutex);
		return ret;
	}
...
#endif
	{
		...
		DDD("hist_stat = 0x%x, new waveform = 0x%x\n",
				...
				struct mxc_epdc_fb_data *fb_data)
{
	...
	{
#if 1 //[
		DDD("%s():wf mode merge %d->%d\n",__FUNCTION__,a->waveform_mode,fb_data->wv_modes.mode_gc16);
		a->waveform_mode = fb_data->wv_modes.mode_gc16;
#else //][!
		DDD("%s():wf mode merge %d->%d",__FUNCTION__,a->waveform_mode,WAVEFORM_MODE_AUTO);
...
static inline bool is_fullscreen_update(int w, int h)
{
	if (w >= g_fb_data->epdc_fb_var.xres && h >= g_fb_data->epdc_fb_var.yres) return true;
	if (w >= g_fb_data->epdc_fb_var.yres && h >= g_fb_data->epdc_fb_var.xres) return true;
	return false;
}
...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	DBG(" -- epdc_submit_work_func: start\n");
	...
	list_for_each_entry_safe(next_update, temp_update,
		...
		DBG("A collision update is ready to go!\n");
		...
		if (!upd_data_list) {
			...
			case MERGE_OK:
				...
				break;
			case MERGE_FAIL:
				DBG("Update not merged [collision]\n");
				break;
			case MERGE_BLOCK:
				DBG("Merge blocked [collision]\n");
				end_merge = true;
				break;
			}
		...
		list_for_each_entry_safe(next_desc, temp_desc,
			...
			if (!upd_data_list) {
				...
				switch (epdc_submit_merge(upd_data_list->update_desc,
						next_desc, fb_data)) {
				case MERGE_OK:
					DBG("Update merged [queue]\n");
					list_del_init(&next_desc->list);
					kfree(next_desc);
					break;
				case MERGE_FAIL:
					DBG("Update not merged [queue]\n");
					break;
				case MERGE_BLOCK:
					DBG("Merge blocked [collision]\n");
					end_merge = true;
	...
	if ((fb_data->epdc_fb_var.rotate == FB_ROTATE_UR) &&
		...
		/* If needed, enable EPDC HW while ePxP is processing */
		if ((fb_data->power_state == POWER_STATE_OFF) || fb_data->powering_down) {
			epdc_powerup(fb_data, 0 /* TODO: voff */);
		}
	 ...
	 */
	DBG("AA: order=%d is_aa=%d wb_pause=%d flags=0x%x\n",
		upd_data_list->update_desc->update_order,	
	...
	if (upd_data_list->update_desc->is_aa &&
		...
		if (do_aa_processing_v2_2_0((uint8_t *)(upd_data_list->virt_addr
			...
			ERR(" AAD algorithm is not available in this EPDC driver!\n");
		...
		if (do_aad_processing_v2_1_0((uint8_t *)(upd_data_list->virt_addr
				...
				fb_data->native_width,
				fb_data->native_height))
			ERR(" AA algorithm is not available in this EPDC driver!\n");
		...
		/* collect the system time data */
		aa_time_stamp[1] = get_uSecs();
		DBG(" --- AA-D(v2.1.0) Algo Exec Time = %d\n", getTimeDiff(aa_time_stamp[0], aa_time_stamp[1]));
				   ...
				   struct fb_info *info)
{
	...
	if ((upd_data->update_mode != UPDATE_MODE_PARTIAL) &&
		...
		ERR("Update mode 0x%x is invalid.  Aborting update.\n", upd_data->update_mode);
		return -EINVAL;
	}
	if ((upd_data->waveform_mode > 255) &&
		(upd_data->waveform_mode != WAVEFORM_MODE_AUTO)) {
		ERR("Update waveform mode 0x%x is invalid. Aborting update.\n", upd_data->waveform_mode);
		return -EINVAL;
	}
	if ((upd_data->update_region.left + upd_data->update_region.width > fb_data->epdc_fb_var.xres) ||
		...
		ERR("Update region is outside bounds of framebuffer. Aborting update.\n");
		return -EINVAL;
	}
	if (upd_data->flags & EPDC_FLAG_USE_ALT_BUFFER) {
		if ((upd_data->update_region.width !=
			...
			ERR("Alternate update region dimensions must match screen update region dimensions.\n");
			return -EINVAL;
		}
		...
		if ((upd_data->alt_buffer_data.phys_addr <
			...
			ERR("Invalid physical address for alternate buffer.  Aborting update...\n");
			return -EINVAL;
		}
	}
	...
	// use snapshot mode for A2
	//upd_scheme = (upd_data->waveform_mode == 4) ? UPDATE_SCHEME_SNAPSHOT : fb_data->upd_scheme;
	upd_scheme = fb_data->upd_scheme;
	...
	if (fb_data->upd_scheme == UPDATE_SCHEME_SNAPSHOT) {
		...
		if (upd_data->update_mode == UPDATE_MODE_FULL && 
				(upd_data->waveform_mode != g_fb_data->wv_modes.mode_a2 && upd_data->waveform_mode != g_fb_data->wv_modes.mode_du)) {
			DBG("Flush updates\n");
			mutex_unlock(&fb_data->queue_mutex);
			mxc_epdc_fb_flush_updates(fb_data);
			mutex_lock(&fb_data->queue_mutex);
		...
		if (count + fb_data->max_num_buffers
			...
			ERR("No free intermediate buffers available.\n");
			mutex_unlock(&fb_data->queue_mutex);
			return -ENOMEM;
		}
	...
	if (!upd_desc) {
		ERR("Insufficient system memory for update! Aborting.\n");
		if (upd_scheme == UPDATE_SCHEME_SNAPSHOT) {
	...
	upd_desc->iDither_state = iDither_state;
			...
			if(fb_data->wv_modes.mode_aad==upd_desc->upd_data.waveform_mode && 
					fb_data->wv_modes.mode_aad!=fb_data->wv_modes.mode_gc16) 
			{
				DBG("AAD flag is set\n");
				upd_desc->upd_data.flags |= EPDC_FLAG_USE_AAD;
			}
			else {
				DBG("AAD flag is clear\n");
				upd_desc->upd_data.flags &= ~EPDC_FLAG_USE_AAD;
			}
						...
						struct fb_info *info)
{
	...
	if (!ret) {
		ERR("Timed out waiting for update completion\n");
		return -ETIMEDOUT;
	}
...
static int mxc_epdc_fb_ioctl(struct fb_info *info, unsigned int cmd,
			     unsigned long arg)
{
	static const char *u_modes[4] = { "PART", "FULL", "PARTHQ", "FULLHQ" };
	...
	case MXCFB_SEND_UPDATE:
		{
			...
			struct mxcfb_update_data upd_data;
			int i, pb_wf_mode, updmode;

			if (!copy_from_user(&upd_data, argp,
				sizeof(upd_data))) {
DBG("<3>%s %d,%d,%d,%d wf=%d %08x\n", u_modes[upd_data.update_mode & 3],
upd_data.update_region.left, upd_data.update_region.top, upd_data.update_region.width, upd_data.update_region.height,
upd_data.waveform_mode, upd_data.update_marker);

				if (upd_data.update_region.width == 0 || upd_data.update_region.height == 0) {
					ret = 0;
					break;
				}

				if (upd_data.update_marker == LOGO_MARKER && ! user_update_issued) {
					logo_in_progress = 1;
				}
				if (upd_data.update_marker !=LOGO_MARKER &&
						(upd_data.update_mode != UPDATE_MODE_PARTIAL || (upd_data.update_region.width > 32 && upd_data.update_region.height > 32))) {
					if (shadow_fb_info) {
						DBG("recovering shadow fb info\n");
						fb_data_shadow->info.lock = g_fb_data->info.lock;
						fb_data_shadow->info.mm_lock = g_fb_data->info.mm_lock;
						memcpy((struct mxc_epdc_fb_data *)g_fb_data, fb_data_shadow, sizeof(struct fb_info));
						mxc_epdc_fb_set_par_really(g_fb_data);
						shadow_fb_info = 0;
					}
					user_update_issued = 1;
					logo_in_progress = 0;
				}
				if (upd_data.update_marker == LOGO_MARKER) {
					if (user_update_issued) {
						ret = -EBUSY;
						break;
					}
					upd_data.update_marker = 0;
				...
				// DmitryZ: convert static pocketbook wf index to waveform mode
				pb_wf_mode = upd_data.waveform_mode;
				updmode = upd_data.update_mode;
				switch (pb_wf_mode) {
					case EPDC_WFTYPE_INIT:   upd_data.waveform_mode = g_fb_data->wv_modes.mode_init; break;
					case EPDC_WFTYPE_DU:     upd_data.waveform_mode = g_fb_data->wv_modes.mode_du; break;
					case EPDC_WFTYPE_GC16:   upd_data.waveform_mode = g_fb_data->wv_modes.mode_gc16; break;
					case EPDC_WFTYPE_GC4:    upd_data.waveform_mode = g_fb_data->wv_modes.mode_gc4; break;
					case EPDC_WFTYPE_A2:     upd_data.waveform_mode = g_fb_data->wv_modes.mode_a2; break;
					case EPDC_WFTYPE_GL16:   upd_data.waveform_mode = g_fb_data->wv_modes.mode_gl16; break;
					case EPDC_WFTYPE_A2IN:   upd_data.waveform_mode = g_fb_data->wv_modes.mode_a2in; break;
					case EPDC_WFTYPE_A2OUT:  upd_data.waveform_mode = g_fb_data->wv_modes.mode_a2out; break;
					case EPDC_WFTYPE_DU4:    upd_data.waveform_mode = g_fb_data->wv_modes.mode_du; break;
					case EPDC_WFTYPE_GC16HQ: upd_data.waveform_mode = g_fb_data->wv_modes.mode_gc16; break;
					case EPDC_WFTYPE_AA:     upd_data.waveform_mode = g_fb_data->wv_modes.mode_aa; break;
					case EPDC_WFTYPE_AAD:    upd_data.waveform_mode = g_fb_data->wv_modes.mode_aad; break;
					default:                 upd_data.waveform_mode = g_fb_data->wv_modes.mode_gc16; break;
				}

				if (updmode == UPDATE_MODE_FULLHQ || (pb_wf_mode == EPDC_WFTYPE_GC16HQ && updmode == UPDATE_MODE_FULL)) {
					DBG("Using AAD\n");
					upd_data.waveform_mode = g_fb_data->wv_modes.mode_aad;
					upd_data.update_mode = UPDATE_MODE_FULL;
				}
				if (updmode == UPDATE_MODE_PARTIALHQ || (pb_wf_mode == EPDC_WFTYPE_GC16HQ && updmode == UPDATE_MODE_PARTIAL)) {
					DBG("Using AA\n");
					upd_data.waveform_mode = g_fb_data->wv_modes.mode_aa;
					upd_data.update_mode = UPDATE_MODE_FULL;
				}

				DBG("WF mode: (%d->%d)\n", pb_wf_mode, upd_data.waveform_mode);
				if (upd_data.waveform_mode == -1) return -EINVAL;

				if (upd_data.update_mode == UPDATE_MODE_FULL || 
				    pb_wf_mode == EPDC_WFTYPE_A2 ||
				    pb_wf_mode == EPDC_WFTYPE_A2IN ||
				    pb_wf_mode == EPDC_WFTYPE_A2OUT
				) {
					DBG("fulsh updates start\n");
     			mxc_epdc_fb_flush_updates(fb_data);
					// wait for updates to complete
					for (i=50; i>0; --i) {
						if (! fb_data->updates_active) break;
						msleep(20);
					}
					DBG("fulsh updates end\n");
				}
...
void mxc_epdc_fb_flush_updates(struct mxc_epdc_fb_data *fb_data)
{
	...
	if (!list_empty(&fb_data->upd_pending_list) ||
		...
		if (!ret)
			ERR("Flush updates timeout! ret = 0x%x\n", ret);
...
static int mxc_epdc_fb_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_POWERDOWN:
		...
		if (fb_data->power_state == POWER_STATE_ON) {
			...
			if (fb_data->updates_active) {
				printk("EPDC busy\n");
				return -EBUSY;
			}

			fb_data->powering_down = 2;
                        /* Schedule task to disable EPDC HW until next update */
			//epdc_done_work_func(&fb_data->epdc_done_work);
		        ...
		        flush_delayed_work(&fb_data->v3p3_off_work);
			fb_data->powering_down = 0;
			//epdc_powerdown(fb_data, true);
		} else
			mutex_unlock(&fb_data->power_mutex);
				   ...
				   struct fb_info *info)
{
	...
	DBG("%s: var->yoffset %d, info->var.yoffset %d\n",
...
static void epdc_done_work_func(struct work_struct *work)
{
	struct mxc_epdc_fb_data *fb_data =
		container_of(work, struct mxc_epdc_fb_data,
			epdc_done_work.work);
	epdc_powerdown(fb_data, (fb_data->powering_down == 2));
}
...
static irqreturn_t mxc_epdc_irq_handler(int irq, void *dev_id)
{
	...
	if (fb_data->in_init) {
		if (epdc_is_working_buffer_complete()) {
			...
			DDD("Cleared WB for init update\n");
		}
		...
		if (epdc_is_lut_complete(fb_data->rev, 0)) {
			...
			DDD("Cleared LUT complete for init update\n");
		}
	...
	if (__raw_readl(EPDC_IRQ) & EPDC_IRQ_TCE_UNDERRUN_IRQ) {
		ERR("TCE underrun! Will continue to update panel\n");
	...
	DDD("EPDC interrupts fired = 0x%x, "
...
static void epdc_intr_work_func(struct work_struct *work)
{
	...
	DDD("int work (A%d B%d C%d AVL%d COL%d W%d IRQ:%08x%08xi PAUSE:%d\n",
		epdc_luts_active,
		epdc_wb_busy,
		epdc_lut_cancelled,
		epdc_luts_avail,
		...
		if (fb_data->cur_update->update_desc->upd_data.flags
			...
			*/
			...
			list_for_each_entry_safe(next_marker, temp,
				...
				/* Signal completion of update */
				DBG("Signaling marker (cancelled) %d\n",
					next_marker->update_marker);
				if (next_marker->waiting)
					complete(&next_marker->update_completion);
				else
			...
			DBG("No pausing this time folks!\n");
		}
		...
		if (epdc_collision & update_is_valid)  {
			...
			for (temp_mask = fb_data->cur_update->collision_mask, lut = 0;
				...
				if (fb_data->lut_update_order[lut] >=
					...
					DBG("Ignoring collision with newer update.\n");
					ignore_collision = true;
					break;
				}
			}
			if (fb_data->cur_update->update_desc->upd_data.waveform_mode != 4 && !ignore_collision) {
				DDD("  collision: %x %d\n", fb_data->cur_update->collision_mask, fb_data->cur_update->update_desc->update_order);
				free_update = false;
				/*
...
static void draw_mode0(struct mxc_epdc_fb_data *fb_data)
{
	...
	epdc_submit_update(0, fb_data->wv_modes.mode_du, UPDATE_MODE_FULL,
		false, false, true, 0xFF);

	DBG("Mode0 update - Waiting for LUT to complete...\n");

	/* Will timeout after ~4-5 seconds */

	for (i = 0; i < 40; i++) {
		if (!epdc_is_lut_active(0)) {
			DBG("Mode0 init complete\n");
			return;
		}
		msleep(100);
	}

	ERR("Mode0 init failed!\n");

	return;
}

static int mxc_epdc_load_waveform(const unsigned char *fw_data, int fw_size,
						     void *context)
{
	...
	wv_file = (struct mxcfb_waveform_data_file *)fw_data;
	...
	fb_data->waveform_buffer_size = fw_size - wv_data_offs;
	INF("  waveform buffer size: %d\n", fb_data->waveform_buffer_size);
...

	{
		...
		u64 longOffset;
		u32 bufferSize;
		u8 *fwDataBuffer = (u8 *)(fw_data) + wv_data_offs;
		...
		DBG("******* wtrc=%d wmc=%d awv=%d xwia=%d longOffset=%llx\n", wtrc, wmc, awv, xwia, longOffset);
	...
	if (fb_data->waveform_buffer_virt == NULL) {
		ERR("Can't allocate mem for waveform!\n");
		return -1;
	}

	DBG("[%s]waveform:vir_p=%p,phy_p=%p,sz=%d,trt_entries=%d\n",__FUNCTION__,\
		fb_data->waveform_buffer_virt,fb_data->waveform_buffer_phys,\
		fb_data->waveform_buffer_size,fb_data->trt_entries);
	
	memcpy(fb_data->waveform_buffer_virt, (u8 *)(fw_data) + wv_data_offs,
		fb_data->waveform_buffer_size);
	...
	if (((rounded_pix_clk >= target_pix_clk + target_pix_clk/100) ||
		...
		if (((rounded_pix_clk >= target_pix_clk + target_pix_clk/100) ||
			...
			ERR("Unable to get an accurate EPDC pix clk"
...
static int mxc_epdc_fb_read_wf_name(char *buf, int maxlen)
{
	mm_segment_t fs;
	struct file *fp = NULL;
	loff_t pos = 0;
	...
	fs = get_fs();
	set_fs(KERNEL_DS);
	fp = filp_open(waveform_filename, O_RDONLY, 0);
	if (! IS_ERR(fp)) {
		pos = waveform_offset;
		n = vfs_read(fp, header, 512, &pos);
		if (n == 512) {
			// anyway, we got something
			ret = 0;
			xwia = header[0x1c] + (header[0x1d] << 8) + (header[0x1e] << 16);
			if (xwia < 512) nlen = header[xwia];
			if (nlen > maxlen-1) nlen = maxlen-1;
			if (nlen >= 1 && xwia+nlen <= 512) {
			        memcpy(buf, &header[xwia+1], nlen);
			        buf[nlen] = 0;
			}
		}
		filp_close(fp, NULL);
...
static int mxc_epdc_fb_init_hw(struct fb_info *info)
{
	...
	if (strncmp(waveform_filename, "/dev", 4) == 0) {
		// reading name from raw device
		pos = waveform_name_offset;
		n = vfs_read(fp, header, 512, &pos);
		if (n != 512) {
			ERR("could not read waveform name\n");
		} else {
			strncpy(waveform_name, header, sizeof(waveform_name)-1);
		}
	} else {
		p = strrchr(waveform_filename, '/');
		if (! p) p = waveform_filename;
		strncpy(waveform_name, p, sizeof(waveform_name)-1);
	}
	for (p=waveform_name; *p; p++) if (*p < ' ') *p = 0;
	...
	data = vmalloc(len+1024);
	if (data == NULL) {
		ERR("could not allocate memory\n");
		goto err2;
	}
	memcpy(data, header, 512);
	n = vfs_read(fp, data+512, (len+512) & ~511, &pos);
	if (n < len-512) {
		ERR("could not read waveform file (%d)\n", n);
		goto err2;
	}

	gbModeVersion = data[0x10];
	gbWFM_REV = data[0x16];
	gbFPL_Platform = data[0x0d];
	gbRegal = 0;
	INF("ModeVersion=%d WFM_rev=%d FPL=%d REGAL=%d\n", gbModeVersion, gbWFM_REV, gbFPL_Platform, gbRegal);

	switch (gbModeVersion) {
		case 0x20:
		case 0x19:
		case 0x18:
			fb_data->wv_modes.mode_gl16 = 3;
			fb_data->wv_modes.mode_a2 = 6;
			fb_data->wv_modes.mode_a2in = 1;
			fb_data->wv_modes.mode_gc4 = (0x18==gbModeVersion) ? 2 : 7;
			fb_data->wv_modes.mode_du = 1;
			fb_data->wv_modes.mode_aa = 4;
			fb_data->wv_modes.mode_aad = 5;
			gbRegal = 1;
			break;
		case 0x13:
			fb_data->wv_modes.mode_du = 1;
		...
		case 0x23:
			...
			break;
		case 0x4:
			fb_data->wv_modes.mode_gl16 = 5;
			fb_data->wv_modes.mode_aa = 5;
			fb_data->wv_modes.mode_aad = 5;
			fb_data->wv_modes.mode_a2 = 4;
			fb_data->wv_modes.mode_a2in = 1;
			break;
		...
		default:
			...
			fb_data->wv_modes.mode_aa = 2;
			fb_data->wv_modes.mode_aad = 2;
			fb_data->wv_modes.mode_a2 = 1;
			break;
	}
	fb_data->wv_modes_update = true;

	INF("Waveform modes: DU=%d GC4=%d GC16=%d GL16=%d AA=%d AAD=%d A2=%d A2IN=%d A2OUT=%d\n",
		fb_data->wv_modes.mode_du, fb_data->wv_modes.mode_gc4,
		fb_data->wv_modes.mode_gc16, fb_data->wv_modes.mode_gl16,
		fb_data->wv_modes.mode_aa, fb_data->wv_modes.mode_aad,
		fb_data->wv_modes.mode_a2,
		fb_data->wv_modes.mode_a2in, fb_data->wv_modes.mode_a2out);

	xwia = data[0x1c] + (data[0x1d] << 8) + (data[0x1e] << 16);
	nlen = data[xwia];
	if (nlen >= 1 && data[xwia+1] != 0) {
		if (nlen > 63) nlen = 63;
		memcpy(waveform_name, &data[xwia+1], nlen);
		waveform_name[nlen] = 0;
		INF("waveform name (from header): %s\n", waveform_name);
	} else {
		INF("waveform name (from mmc): %s\n", waveform_name);
	}

	udata = vmalloc(WF_MAX_SIZE);
	if (! udata) {
		ERR("could not allocate memory for waveform\n");
		goto err2;
	}
	outlen = WF_MAX_SIZE;
	ret = wbf_convert(data, len, udata, &outlen, 1);
	if (ret != 0) {
		ERR("could not unpack waveform (%d)\n", ret);
		goto err2;
	}
	INF("unpacked waveform size: %d\n", outlen);

	ret = mxc_epdc_load_waveform(udata, outlen, fb_data);
	if (ret) {
		ERR("mxc_epdc_load_waveform: error\n");
		goto err2;
	}
...
#ifdef READ_VCOM_FROM_FILE

	fp = filp_open(vcom_filename, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		fp = NULL;
		ERR("Could not read VCOM value\n");
		goto no_vcom;
	}
	...
	if (*vp == '-') vp++;
	vcom_value = simple_strtoul(vp, NULL, 10);
no_vcom:

#endif
	if (vcom_value < 500 || vcom_value > 4000) {
		if (tps65185_vcom_get(&vcom_value) == 0) {
			if (vcom_value < 0) vcom_value = -vcom_value;
			INF("tps65185_vcom_get: VCOM=%d", vcom_value);
		} else {
			vcom_value = 0;
		}
	}
	...
	ret = mxc_epdc_fb_wait_update_complete(&upd_marker_data, &fb_data->info);
	if (ret < 0)
		ERR("Wait for initial update complete failed. Error = 0x%x", ret);
#endif
...
static ssize_t update_store(struct device *device,
...
static ssize_t vcom_setup(const char *buf, size_t count, int perm)
{
	...
	while (*p == ' ' || *p == '\t') p++;
	if (*p != '-' || *(p+1) < '0' || *(p+1) > '4') return -EINVAL;
	vcom = simple_strtoul(p+1, NULL, 10);
	if (vcom < 500 || vcom > 4500) return -EINVAL;
	vcom_value = vcom;
	return (tps65185_vcom_set(vcom, perm) == TPS65185_RET_SUCCESS) ? count : -EIO;
}
...
{
	int vcom=0;
	int temp = mxc_epdc_fb_read_cur_temperature(g_fb_data);
    	return  scnprintf(buf, PAGE_SIZE, "%d\n", temp);
}

static ssize_t regal_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    	return  scnprintf(buf, PAGE_SIZE, "%d\n", gbRegal);
}
...
EPDC_RW_ATTRIBUTE(vcom, 0644);
EPDC_WO_ATTRIBUTE(vcom_permanent, 0222);
EPDC_RO_ATTRIBUTE(temperature, 0444);
EPDC_RO_ATTRIBUTE(regal, 0444);
...
static int epdc_probe_thread(void *p_arg)
{
	struct platform_device *pdev = (struct platform_device *) p_arg;
	...
	fb_data_shadow = (struct mxc_epdc_fb_data *) kzalloc(sizeof(struct mxc_epdc_fb_data), GFP_KERNEL);
	if (fb_data == NULL || fb_data_shadow == NULL) {
		ret = -ENOMEM;
		goto out;
	}
	...
	INF("waiting for filesystem access...\n");
	for (i=0; i<10; i++) {
		if (mxc_epdc_fb_read_wf_name(wfname, sizeof(wfname)) == 0) break;
		msleep(100);
	}
	INF("waveform name: %s\n", wfname);
	...
	if (wfname[0]) {
		for (i = 0; i < fb_data->pdata->num_modes; i++) {
			char c = ' ';
			if (strstr(wfname, fb_data->pdata->epdc_mode[i].vmode->name)) {
				fb_data->cur_mode = &fb_data->pdata->epdc_mode[i];
				c = '*';
			}
			INF("%c %s %dx%d\n", c, fb_data->pdata->epdc_mode[i].vmode->name,
				fb_data->pdata->epdc_mode[i].vmode->xres,
				fb_data->pdata->epdc_mode[i].vmode->yres);
		}/*end of for*/
	  }/*end of (if(panel_str))*/
	...
	INF("Video mode: %s %dx%d bpp %d\n", vmode->name, vmode->xres, vmode->yres, fb_data->default_bpp);

	/* Allocate color map for the FB */
	ret = fb_alloc_cmap(&info->cmap, 256, 0);
	if (ret) {
		ERR("fb_alloc_cmap: error\n");
		goto out_fbdata;
	}
	...
	if (info->screen_base == NULL) {
		ERR("dma_alloc_writecombine: error\n");
		ret = -ENOMEM;
		goto out_mapregs;
	}
	INF("  allocated at %p:0x%x\n", info->screen_base,
		fb_data->phys_start);

	memset(info->screen_base, 0xff, fb_data->map_size);
	...
	for (i = 0; i < fb_data->max_num_updates; i++) {
		...
		if (upd_list == NULL) {
			ERR("No memory to allocate update buffers\n");
			ret = -ENOMEM;
			goto out_upd_lists;
		}
	...
	for (i = 0; i < fb_data->max_num_buffers; i++) {
		...
		if (fb_data->virt_addr_updbuf[i] == NULL) {
			ERR("No memory to allocate PxP output buffer (%d bytes)\n", fb_data->max_pix_size);
			ret = -ENOMEM;
			goto out_upd_buffers;
		}

		INF("allocated %d bytes @ 0x%08X\n",
	...
	if (fb_data->virt_addr_copybuf == NULL) {
		ERR("Can't allocate mem for PxP buf!\n");
		ret = -ENOMEM;
		goto out_upd_buffers;
	}
	...
	if (fb_data->working_buffer_A_virt == NULL) {
		ERR("Can't allocate mem for working buf A!\n");
		ret = -ENOMEM;
		goto out_copybuffer;
	}
	memset(fb_data->working_buffer_A_virt, 0, fb_data->working_buffer_size);
	...
	if (fb_data->working_buffer_B_virt == NULL) {
		ERR("Can't allocate mem for working buf B!\n");
		ret = -ENOMEM;
		goto out_dma_work_buf_A;
	}
	memset(fb_data->working_buffer_B_virt, 0, fb_data->working_buffer_size);
	...
	fb_data->wv_modes.mode_a2in = 4; /* default A2IN mode */
	fb_data->wv_modes.mode_a2out = -1; /* default A2OUT mode */
	...
	INIT_DELAYED_WORK(&fb_data->v3p3_off_work, v3p3_off_work_func);
	...
	ret = request_irq(fb_data->epdc_irq, mxc_epdc_irq_handler, 0,
			"fb_dma", fb_data);
	if (ret) {
		ERR("request_irq (%d) failed with error %d\n", fb_data->epdc_irq, ret);
		ret = -ENODEV;
		goto out_dma_work_buf_B;
	}
	...
	ret = tps65185_init(2,1);
	if (ret) {
		ERR("TPS65185 init error\n");
		ret = -ENODEV;
	}
	...
	if (fb_data->pxp_conf.proc_data.lut_map == NULL) {
		ERR("Can't allocate mem for lut map!\n");
		ret = -ENOMEM;
		goto out_regulator3;
	}
	for (i = 0; i < 256; i++)
	...
	fb_data->wait_for_powerdown = false;
	fb_data->updates_active = false;
	fb_data->pwrdown_delay = 100;

	/* Register FB */
	ret = register_framebuffer(info);
	if (ret) {
		ERR("register_framebuffer failed with error %d\n", ret);
		goto out_lutmap;
	}
...
out_fbdata:
	kfree(fb_data);
out:
	INF("mxc_epdc_fb_probe: exited with return code %d\n", ret);
	return ret;
}

int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	struct task_struct *probe_task = kthread_create(epdc_probe_thread, pdev, "epdc_probe");
...
static int mxc_epdc_fb_resume(struct platform_device *pdev)
{
	...
	DBG("mxc_epdc_fb_resume\n");
	data->pwrdown_delay = saved_pwrdown_delay;	
...
static int pxp_chan_init(struct mxc_epdc_fb_data *fb_data)
{
	...
	if (!chan) {
		ERR("Unsuccessfully received channel!!!!\n");
		return -EBUSY;
	}
			      ...
			      struct mxcfb_rect *update_region)
{
	...
	DDD("pxp_process_update: starting PxP Send Buffer\n");
	...
	DDD("device_prep_slave_sg\n");
	txd = dma_chan->device->device_prep_slave_sg(dma_chan, sg, 2,
						     DMA_TO_DEVICE,
						     DMA_PREP_INTERRUPT);
	if (!txd) {
		ERR("Error preparing a DMA transaction descriptor.\n");
		return -EIO;
	}
	...
	if (cookie < 0) {
		ERR("Error sending FB through PxP\n");
		return -EIO;
	}
...
static int pxp_complete_update(struct mxc_epdc_fb_data *fb_data, u32 *hist_stat)
{
	 ...
	 */
	DDD("pxp_complete_update start\n");
	ret = wait_for_completion_timeout(&fb_data->pxp_tx_cmpl, HZ / 10);
	if (ret <= 0) {
		INF("PxP operation failed due to %s\n",
