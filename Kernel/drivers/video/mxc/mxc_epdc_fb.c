...
#define MAXWFCOUNT 16
#define MAXTEMPCOUNT 32
#define WF_MAX_SIZE (1024*1024)

#define TEMP_CORRECTION (-2)

static unsigned char *wfaddr[MAXWFCOUNT][MAXTEMPCOUNT];
static int wfsize[MAXWFCOUNT][MAXTEMPCOUNT];
static int maintable[MAXWFCOUNT*2];
static int localtable[MAXWFCOUNT][MAXTEMPCOUNT*2];
static int vcomoffset[MAXWFCOUNT][MAXTEMPCOUNT];

static const char *waveform_filename = "/dev/mmcblk0";
static const int waveform_offset = 10240 * 512;

static const char *vcom_filename = "/dev/mmcblk0";
static const int vcom_offset = 30723 * 512;
...
struct mxc_epdc_fb_data {
	...
	struct regulator *vcomp_regulator;
	struct regulator *v3p3_regulator;
	
	struct regulator *tmst_regulator;/*add for temperature */
...
struct waveform_data_header {
	...
	unsigned int reserved0_0:8;	// 0x27
...
static void vcom_set_voltage(struct mxc_epdc_fb_data *fb_data, int vcom_value, int vcomoffset)
{
	if (vcom_value == 0) {
		...
		int v = ((-vcom_value) + vcomoffset) * 1000;
		if (regulator_set_voltage(fb_data->vcom_regulator, v, v) == 0) {
			INF("VCOM=-%d(%d)\n", vcom_value, vcomoffset);
		} else {
			ERR("Could not set VCOM to -%d\n", vcom_value);
		}
	}
}
...
static void epdc_powerup(struct mxc_epdc_fb_data *fb_data, int vcomoffset)
{
	...
	ret = fb_data->v3p3_on ? 0 : regulator_enable(fb_data->v3p3_regulator);
	fb_data->v3p3_on = 1;
	if (IS_ERR((void *)ret)) {
	...
	ret = fb_data->vcom_on ? 0 : regulator_enable(fb_data->vcom_regulator);
	fb_data->vcom_on = 1;
	if (IS_ERR((void *)ret)) {
...
static void epdc_powerdown(struct mxc_epdc_fb_data *fb_data, bool sync3v3)
{
	...
	if (fb_data->vcom_on) regulator_disable(fb_data->vcom_regulator);
	fb_data->vcom_on = 0;
	...
	if (sync3v3) {
		mdelay(50);
		if (fb_data->v3p3_on) regulator_disable(fb_data->v3p3_regulator);
		mdelay(10);
		fb_data->v3p3_on = 0;
	} else {
...
int mxc_epdc_fb_read_temperature(struct mxc_epdc_fb_data *fb_data)
{
	...
	if(!IS_ERR(fb_data->tmst_regulator))
	{
		...
		{
			temperature = regulator_get_voltage(fb_data->tmst_regulator) + TEMP_CORRECTION;
			dev_dbg(fb_data->dev, "auto temperature reading = %d\n", temperature );
			//printk("%s %s %d temperature=%d \n",__FILE__,__func__,__LINE__,temperature);
			if (temperature != 0xFF) {
...
DDD("-- epdc_process_update: start pxp\n"); 
...
DDD("-- epdc_process_update: epdc power-on\n");
	int voff = vcomoffset[upd_desc_list->upd_data.waveform_mode][fb_data->temp_index];

	/* If needed, enable EPDC HW while ePxP is processing */
	if ((fb_data->power_state == POWER_STATE_OFF)
		|| fb_data->powering_down) {
		epdc_powerup(fb_data, voff);
	}

DDD("-- epdc_process_update: wait...\n");
	/* This is a blocking call, so upon return PxP tx should be done */
...
DDD("-- epdc_process_update: pxp complete\n");
...
DDD(" -- epdc_submit_work_func: start\n");
...
DDD("-- epdc_submit_work_func: processing\n");
	/*
...
DDD("-- rotate=%d grayscale=%d is_transform=%d rev=%d restrict_width=%d\n", fb_data->epdc_fb_var.rotate, fb_data->epdc_fb_var.grayscale, is_transform, fb_data->rev, fb_data->restrict_width);
...
DDD("-- epdc_submit_work_func: no pxp used\n");

		/* If needed, enable EPDC HW while ePxP is processing */
		if ((fb_data->power_state == POWER_STATE_OFF) || fb_data->powering_down) {
		        int voff = vcomoffset[upd_data_list->update_desc->upd_data.waveform_mode][fb_data->temp_index];
			epdc_powerup(fb_data, voff);
		}
...
DDD("-- epdc_submit_work_func: pxp used\n");
...
DDD("-- epdc_submit_work_func: read temperature\n");
	/* Check if auto temperature update is needed */
	mxc_epdc_fb_read_temperature(fb_data);

DDD("-- epdc_submit_work_func: adjust coordinates\n");
...
DDD("-- epdc_submit_work_func: wait wb busy\n");
	/*
...
DDD("-- epdc_submit_work_func: dithering\n");
	/*
...
DDD("-- epdc_submit_work_func: wait lut\n");
	/*
...
DDD("-- epdc_submit_work_func: get lut\n");
...
DDD("-- epdc_submit_work_func: submit update\n");
...
DDD("-- mxc_epdc_fb_send_single_update: %d %s\n", upd_data->waveform_mode, upd_data->update_mode == UPDATE_MODE_FULL ? "full" : "partial");
	...
	if (fb_data->upd_scheme == UPDATE_SCHEME_SNAPSHOT) {
		 ...
		 */
		if (upd_data->update_mode == UPDATE_MODE_FULL && upd_data->waveform_mode != 4) {
			mutex_unlock(&fb_data->queue_mutex);
			mxc_epdc_fb_flush_updates(fb_data);
			mutex_lock(&fb_data->queue_mutex);
		}
...
DDD("-- mxc_epdc_fb_send_single_update: allocate memory\n");
...
DDD("-- mxc_epdc_fb_send_single_update: queue work\n");
...
DDD("-- mxc_epdc_fb_send_single_update: snapshot update\n");
...
DDD("-- mxc_epdc_fb_send_single_update: snapshot complete\n");
	mutex_unlock(&fb_data->queue_mutex);
	return 0;
}
	...
	case MXCFB_SEND_UPDATE:
		{
			...
			if (!copy_from_user(&upd_data, argp,
				sizeof(upd_data))) {
DDD("%s %d,%d,%d,%d wf=%d\n", upd_data.update_mode == UPDATE_MODE_FULL ? "FULL" : "PART",
...
upd_data.waveform_mode);
				if (upd_data.update_mode == UPDATE_MODE_FULL || 
				    upd_data.waveform_mode == 4 ||
				    upd_data.waveform_mode == 6 ||
				    upd_data.waveform_mode == 7
				) {
                			mxc_epdc_fb_flush_updates(fb_data);
...
static int mxc_epdc_fb_blank(int blank, struct fb_info *info)
{
	...
	dev_dbg(fb_data->dev, "blank = %d,fb_data->blank %d\n", blank,fb_data->blank );
...
#define A2S_BB_RATIO 200
#define A2S_BB_BALANCE 65
#define A2S_BW_RATIO 220

#define A2E_BB_RATIO 70

#define A2E_WW_BALANCE 0
#define A2E_WW_RATIO 100
...
static int generate_a2_wf(int ntemp)
{
    unsigned char *tbuf;
    int i, k, steps, bw_steps;

    for (i=0; i<ntemp; i++) {

        bw_steps = wfsize[EPDC_WFTYPE_DU][i] / 256;
        steps = (bw_steps * A2_RATIO) / 100;
	wfsize[EPDC_WFTYPE_A2][i] = steps * 256;
        wfaddr[EPDC_WFTYPE_A2][i] = vmalloc(256 * (steps+1));

	tbuf = wfaddr[EPDC_WFTYPE_A2][i];
        memset(tbuf, 0, 256 * (steps+1));

        for (k=0; k<steps-1; k++) {
            tbuf[k*256 + 15] = 0x01;
            tbuf[k*256 + 15 * 16] = 0x02;
        }
    }
    return 0;
}

static int generate_a2plus_wf(int ntemp)
{
    unsigned char *tbuf;
    int i, j, k, steps;
    int a2_steps, du_steps, bb_steps, bw_steps, ww_steps, ww_shuffle;
    int ns, nb, nw;

    // creating a2s waveform

    for (i=0; i<ntemp; i++) {

        a2_steps = wfsize[EPDC_WFTYPE_A2][i] / 256;
        bb_steps = (a2_steps * A2S_BB_RATIO) / 100;
        bw_steps = (a2_steps * A2S_BW_RATIO) / 100;
        steps = (bb_steps > bw_steps) ? bb_steps : bw_steps;
	wfsize[EPDC_WFTYPE_A2IN][i] = steps * 256;
        wfaddr[EPDC_WFTYPE_A2IN][i] = vmalloc(256 * (steps+1));

	tbuf = wfaddr[EPDC_WFTYPE_A2IN][i];
        memset(tbuf, 0, 256 * (steps+1));

        for (j=0; j<16; j++) {
            ns = ((a2_steps * j) + (bb_steps * (15 - j))) / 15 + 1;
            if (ns < 1) ns = 1;
            if (ns > steps) ns = steps;
            nb = ((a2_steps * j) + (((bb_steps * (100-A2S_BB_BALANCE)) / 100) * (15 - j))) / 15 + 1;
            for (k=steps-ns; k<steps-1; k++) {
                tbuf[k * 256 + j] = (steps-k > nb) ? 0x02 : 0x01;
            }
        }

        for (j=0; j<16; j++) {
            ns = (bw_steps * (15 - j)) / 15 + 1;
            if (j < 15) ns += (bw_steps * j) / (15 * 15);
            if (ns > steps) ns = steps;
            for (k=steps-ns; k<steps-1; k++) {
                tbuf[k * 256 + 15 * 16 + j] = 0x02;
                nw--;
            }
        }
    }

    // creating a2e waveform

    for (i=0; i<ntemp; i++) {

        du_steps = wfsize[EPDC_WFTYPE_DU][i] / 256;
        a2_steps = wfsize[EPDC_WFTYPE_A2][i] / 256;
        bb_steps = (du_steps * A2E_BB_RATIO) / 100;
        ww_steps = (du_steps * A2E_WW_RATIO) / 100;
        ww_shuffle = (ww_steps * A2E_WW_BALANCE) / 100;
        if (ww_steps < 2) ww_steps = 2;
        steps = (bb_steps > ww_steps) ? bb_steps : ww_steps;
        if (steps < a2_steps) steps = a2_steps;
        wfsize[EPDC_WFTYPE_A2OUT][i] = steps * 256;
        wfaddr[EPDC_WFTYPE_A2OUT][i] = vmalloc(256 * (steps+1));

	tbuf = wfaddr[EPDC_WFTYPE_A2OUT][i];
        memset(tbuf, 0, 256 * (steps+1));

        for (k=steps-bb_steps; k<steps-1; k++) {
            tbuf[k * 256] = 0x01;
        }

        ww_shuffle = ww_steps / 2;
        for (k=steps-ww_steps-1; k<steps-1; k++) {
            if (k < 0) continue;
            tbuf[k * 256 + 255] = (ww_shuffle > 0) ? 0x01 : 0x02;
            ww_shuffle--;
        }

        for (k=0; k<a2_steps-1; k++) {
            tbuf[k*256 + 15] = 0x01;
            tbuf[k*256 + 15 * 16] = 0x02;
        }
    }
...
static int read_waveform_data(unsigned char *wfdata, int off, unsigned char *out, int maxlen) {
...
static inline int is_a2_waveform(unsigned char *data, int len)
{
	// for A2 waveform, there should not be changed
	// pixels except 0-15 and 15-0
	int i;
	for (i=0; i<len/256; i+=256) {
		if (data[i*256+0x1e] != 0) return 0;
		if (data[i*256+0xe1] != 0) return 0;
	}
	return 1;
}

static inline void fw_write(void *data, int len, void *out, int *pos)
{
...
static unsigned char *unpack_waveform(unsigned char *wfdata, int inlen, int *outlen)
{
	unsigned char *tbuf = vmalloc(WF_MAX_SIZE);
	unsigned char *outdata = NULL;
	int sizebuf[2];

	int wfcount = wfdata[0x25] + 1;
	int tempcount = wfdata[0x26] + 1;
	int vcinfo = wfdata[0x27];

	unsigned char *sp, *dp, *addr;
	int wf, wfd, temp, offs, offs1, offs2, offs2next, len, pos, cs;
	int i, x, y, nb, is5bit, vcoffs;
	...
	if (wfcount > 7) wfcount = 7;
	is5bit = ((wfdata[0x24] & 0xc) == 0x4);

	for (wf=1; wf<wfcount; wf++) {
		for (temp=0; temp<tempcount; temp++) {
			if (wf == 0) {
				...
				vcoffs = (vcinfo == 1 || vcinfo == 3) ? offs2 - 16 : 0;
				len = read_waveform_data(wfdata, offs2, tbuf, WF_MAX_SIZE);
			}
			wfd = (wf == 6) ? 4 : wf;
			if (is5bit) {
				// 5-bit waveform
				wfaddr[wfd][temp] = vmalloc(len / 4);
				sp = tbuf;
				dp = wfaddr[wfd][temp];
				nb = len;
				while (nb >= 1024) {
				...
				wfsize[wfd][temp] = len / 4;
			} else {
				wfaddr[wfd][temp] = vmalloc(len);
				memcpy(wfaddr[wfd][temp], tbuf, len);
				wfsize[wfd][temp] = len;
			}
			vcomoffset[wfd][temp] = vcoffs ? voltage(wfdata, vcoffs+10, 3125) : 0;
			//vcomoffset[wfd][temp] = (temp - 7) * 150;
			//INF(" wf=%d temp=%d offs=%x len=%d vcomof=%d\n", wfd, temp, offs, len, vcomoffset[wfd][temp]);
		}
	}

	if (wfcount < 5) {
		// no A2 waveform, but we need it!
		generate_a2_wf(tempcount);
		wfcount = 5;
	}

	// if waveform 4 looks not like A2, use wf 6 instead
	if (wfcount >= 7 && !is_a2_waveform(wfaddr[4][0], wfsize[4][0])) {
		for (temp=0; temp<tempcount; temp++) {
			addr = wfaddr[4][0];
			len = wfsize[4][0];
			wfaddr[4][0] = wfaddr[6][0];
			wfsize[4][0] = wfsize[6][0];
			wfaddr[6][0] = addr;
			wfsize[6][0] = len;
		}
	}
	...
	if (wfcount >= 5) {
		for (temp=0; temp<tempcount; temp++) {
			if (wfaddr[6][temp]) vfree(wfaddr[6][temp]);
			wfaddr[6][temp] = NULL;
			if (wfaddr[7][temp]) vfree(wfaddr[7][temp]);
			wfaddr[7][temp] = NULL;
		}
		if (generate_a2plus_wf(tempcount) == 0) wfcount = 8;
	}		

	// ensure there are no unfilled entries
	for (wf=0; wf<wfcount; wf++) {
		for (temp=0; temp<tempcount; temp++) {
			if (wfaddr[wf][temp] == NULL) {
				wfaddr[wf][temp] = vmalloc(256);
				memset(wfaddr[wf][temp], 0, 256);
				wfsize[wf][temp] = 256;
			}
		}
	}
	...
	for (wf=0; wf<wfcount; wf++) {
		...
		offs += 8 * tempcount;
		for (temp=0; temp<tempcount; temp++) {
			localtable[wf][temp*2] = offs;
			offs += 8;
			offs += wfsize[wf][temp];
		}
	}

	*outlen = (1024 + offs) & ~511;
	outdata = vmalloc(*outlen);
	if (outdata == NULL) {
		ERR("could not allocate %d bytes for waveform data\n", *outlen);
		goto err1;
	}
	pos = 0;

	fw_write(wfdata, 0x30, outdata, &pos);
	fw_write(wfdata+0x30, tempcount+1, outdata, &pos);

	fw_write(maintable, 8 * wfcount, outdata, &pos);

	for (wf=0; wf<wfcount; wf++) {
		fw_write(localtable[wf], 8 * tempcount, outdata, &pos);
		for (temp=0; temp<tempcount; temp++) {
			sizebuf[0] = wfsize[wf][temp] / 256;
			sizebuf[1] = 0;
			fw_write(sizebuf, 8, outdata, &pos);
			fw_write(wfaddr[wf][temp], wfsize[wf][temp], outdata, &pos);
		}
	}
...
err1:
	for (wf=0; wf<wfcount; wf++) {
		for (temp=0; temp<tempcount; temp++) {
			if (wfaddr[wf][temp]) vfree(wfaddr[wf][temp]);
			wfaddr[wf][temp] = NULL;
		}
	}
	vfree(tbuf);
...
static int mxc_epdc_fb_init_hw(struct fb_info *info)
{
	...
	if (strncmp(waveform_filename, "/dev", 4) == 0) {
		// reading from raw device: [0] wf name, [512] wf data
	...
	udata = unpack_waveform(data, len, &outlen);
	if (! udata) {
...
int __devinit mxc_epdc_fb_probe(struct platform_device *pdev)
{
	...
	fb_data->vcomp_regulator = regulator_get(NULL, "VCOMP");
	if (IS_ERR(fb_data->vcomp_regulator)) {
		dev_err(&pdev->dev, "Unable to get VCOMP regulator."
			"err = 0x%x\n", (int)fb_data->vcomp_regulator);
	}
	fb_data->v3p3_regulator = regulator_get(NULL, "V3P3");
	if (IS_ERR(fb_data->v3p3_regulator)) {
		regulator_put(fb_data->vcom_regulator);
		regulator_put(fb_data->vcomp_regulator);
		regulator_put(fb_data->display_regulator);
