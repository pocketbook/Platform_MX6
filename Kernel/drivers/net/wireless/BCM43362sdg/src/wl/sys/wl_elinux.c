...
static int wl_elinux_set_pno_setup(struct net_device *dev, char *command, int total_len)
{
	...
	for (i=0; i < nssid; i++) {
		DHD_INFO(("%s: [%d] SSID: %s\n", __FUNCTION__, i, ssidnet[i].ssid.SSID));
		DHD_INFO(("%s: [%d] SSID_len: %d\n", __FUNCTION__, i, ssidnet[i].ssid.SSID_len));
		DHD_INFO(("%s: [%d] flags: %x\n", __FUNCTION__, i, ssidnet[i].flags));
		DHD_INFO(("%s: [%d] infra: %x\n", __FUNCTION__, i, ssidnet[i].infra));
		DHD_INFO(("%s: [%d] auth: %x\n", __FUNCTION__, i, ssidnet[i].auth));
		DHD_INFO(("%s: [%d] wpa_auth: %x\n", __FUNCTION__, i, ssidnet[i].wpa_auth));
		DHD_INFO(("%s: [%d] wsec: %x\n", __FUNCTION__, i, ssidnet[i].wsec));
	}
	DHD_INFO(("%s: scan interval: %d sec\n", __FUNCTION__, pno_interval));
	DHD_INFO(("%s: interval repeat: %d times\n", __FUNCTION__, pno_repeat));
	DHD_INFO(("%s: exponent max interval: %d x 2^%d sec\n", __FUNCTION__, pno_interval, pno_expo_max));
	DHD_INFO(("%s: network lost time: %d sec\n", __FUNCTION__, pno_lost_time));

	res = dhd_dev_pno_set_ex(dev, ssidnet, nssid, pno_interval, pno_repeat, pno_expo_max, pno_lost_time);
...
int wl_elinux_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (strnicmp(command, DRV_CMD_PM_1, strlen(DRV_CMD_PM_1)) == 0) {
...
#if defined(PNO_SUPPORT) && !defined(WL_SCHED_SCAN)
	else if (strnicmp(command, DRV_CMD_PNO_SSIDCLR, strlen(DRV_CMD_PNO_SSIDCLR)) == 0) {
		bytes_written = dhd_dev_pno_reset(net);
	}
	else if (strnicmp(command, DRV_CMD_PNO_SETUP, strlen(DRV_CMD_PNO_SETUP)) == 0) {
		bytes_written = wl_elinux_set_pno_setup(net, command, priv_cmd.used_len);
	}
	else if (strnicmp(command, DRV_CMD_PNO_ENABLE, strlen(DRV_CMD_PNO_ENABLE)) == 0) {
		bytes_written = dhd_dev_pno_enable(net, 1);
	}
	else if (strnicmp(command, DRV_CMD_PNO_DISABLE, strlen(DRV_CMD_PNO_DISABLE)) == 0) {
		bytes_written = dhd_dev_pno_enable(net, 0);
	}
#ifdef PNO_SET_DEBUG
	else if (strnicmp(command, DRV_CMD_PNO_DEBUG, strlen(DRV_CMD_PNO_DEBUG)) == 0) {
		if (priv_cmd.total_len >= sizeof(pno_setup_example)) {
			...
			memcpy(command, pno_setup_example, sizeof(pno_setup_example));
...
#endif /* PNO_SET_DEBUG */
#endif
	else if (strnicmp(command, DRV_CMD_MSGLEVEL, strlen(DRV_CMD_MSGLEVEL)) == 0) {
		int msglevel;
		msglevel = bcm_strtoul(command + strlen(DRV_CMD_MSGLEVEL) + 1, NULL, 0);
		DHD_ERROR(("%s: msg level: 0x%x -> 0x%x\n", __func__, dhd_msg_level, msglevel));
