 ...
 *
 * $Id: wlu.c,v 1.1084.2.43 2011-02-10 22:46:22 $
 */
...
#ifdef WLDSTA
static cmd_func_t wl_dsta_ifadd;
#endif
...
cmd_t wl_cmds[] = {
	...
	"<amode> is one of:\n"
	"\topen, shared, openshared, wpa, wpapsk, wpa2, wpa2psk, wpanone, ftpsk\n"
...
#endif /* WLP2P */
#ifdef WLDSTA
	{ "dsta_ifadd", wl_dsta_ifadd, -1, WLC_SET_VAR,
	"add DSTA interface\n"
	"\tUsage: wl dsta_ifadd <MAC-address> \n"
	"MAC-address: xx:xx:xx:xx:xx:xx"
	},
#endif /* WLDSTA */
...
wlu_srvar(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (ret < 0)
		printf("Error %d: cannot get 'cis_source'\n", ret);
...
wl_tssical_params(void *wl, cmd_t *cmd, char **argv)
{
		...
		(paparams_new[2] == 1) && (paparams_new[3] == 1))
	{
		...
		B0_new = (int16) ((paparams_new[0] << 8)/paparams_new[3]);
		B1_new = (int16) ((paparams_new[1] << 12)/paparams_new[3]);
		A1_new = (int16) ((paparams_new[2] << 15)/paparams_new[3]);
...
wl_ether_atoe(const char *a, struct ether_addr *n)
{
	...
	for (;;) {
		n->octet[i++] = (uint8)strtoul((char *)a, &c, 16);
...
wl_atoip(const char *a, struct ipv4_addr *n)
{
	...
	for (;;) {
		n->addr[i++] = (uint8)strtoul((char *)a, &c, 0);
...
wl_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	PRVAL(rxbadplcp); PRVAL(rxcrsglitch); PRVAL(bphy_rxcrsglitch);
...
#ifdef WLDSTA
static int
wl_dsta_ifadd(void *wl, cmd_t *cmd, char **argv)
{
	wl_dsta_if_t ifreq;
	int count;
