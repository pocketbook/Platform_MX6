...
0. To rebuild dhd driver module for sdio:
  % extract driver-*.tar.gz package on a linux system
  % cd src/dhd/linux
  % make dhd-cdc-sdmmc-gpl

   The resulting module file (dhd.ko) will be placed in a subdirectory
...
1. To rebuild the wl driver module:
  % extract wldriver-*.tar.gz package on a linux system
  % cd src/wl/linux
  % make apdef-stadef
