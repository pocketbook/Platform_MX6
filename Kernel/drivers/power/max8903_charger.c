...
/* fake capacity */
static inline void max8903_get_cap(struct max8903_data *data)
{
	data->cap = 90;
		...
		union power_supply_propval *val)
{
	...
	printk("%s,psp=%d,data->pdata->chg=%d\n",__func__,psp,data->pdata->chg);
...
static void max8903_work(struct work_struct *work)
{
	...
	max8903_get_cap(data);
	printk("%s %s %d \n",__FILE__,__func__,__LINE__); 
	schedule_delayed_work(&data->work, MAX8903_DELAY);
}

static irqreturn_t max8903_charger_dcin(int irq, void *_data)
{
	...
	printk("%s %s %d ta_in=%d data->ta_in=%d\n",__FILE__,__func__,__LINE__,ta_in,data->ta_in); 
...
static irqreturn_t max8903_charger_usbin(int irq, void *_data)
{
	struct max8903_data *data = _data;
	struct max8903_pdata *pdata = data->pdata;
	bool usb_in;
...
static irqreturn_t max8903_charger_fault(int irq, void *_data)
{
	struct max8903_data *data = _data;
	struct max8903_pdata *pdata = data->pdata;
	bool fault;
...
static __devinit int max8903_probe(struct platform_device *pdev)
{
		...
		if (pdata->dok && gpio_is_valid(pdata->dok) &&
			...
			if (error < 0) {
				dev_err(dev, "failed to configure"
					" request/direction for GPIO %d, error %d %d\n",
					gpio, error,__LINE__);
				goto err;
			}
	...
	if (pdata->usb_valid) {
		if (pdata->uok && gpio_is_valid(pdata->uok)) {
			gpio = pdata->uok;
			///uok and dok is the same gpio,so we can 
			///comment below code. we had setted the usb_valid is false.
	...
	data->psy.get_property = max8903_charger_get_property;
...
static int max8903Charger_suspend(struct platform_device *pdev,
				  pm_message_t state)
{
	//int icount =0;
	//do{
...
static int max8903Charger_resume(struct platform_device *pdev)
{
	return 0;
...
static struct platform_driver max8903_driver = {
	.probe	= max8903_probe,
	//.suspend  = max8903Charger_suspend,
	//.resume  = max8903Charger_resume,
	.remove	= __devexit_p(max8903_remove),
	.driver = {
		.name	= MAX8903_DRIVE_NAME,
		.owner	= THIS_MODULE,
	},
};
