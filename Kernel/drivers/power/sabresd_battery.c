...
u32 sabresd_calibrate_battery_capability_percent(struct max8903_data *data)
{
    u8 i;
...
static enum power_supply_property sabresd_max8903_charger_props[] = {
	POWER_SUPPLY_PROP_ONLINE,
};

static enum power_supply_property sabresd_max8903_battery_props[] = {
...
#endif
static void sabresd_max8903_charger_update_status(struct max8903_data *data)
{
	...
	pr_debug("%s,chg: %d\n", __func__,gpio_get_value(data->pdata->chg));
	pr_debug("%s,dc: %d\n",  __func__,gpio_get_value(data->pdata->dok));
	pr_debug("%s,flt: %d\n",  __func__,gpio_get_value(data->pdata->flt));
}
...
u32 sabresd_calibration_voltage(struct max8903_data *data)
{
...
#endif
static void sabresd_max8903_battery_update_status(struct max8903_data *data)
{
	int temp = 0;
	...
	if (!data->pdata->feature_flag) {
#ifdef CONFIG_TOUCHSCREEN_MAX11801
		temp = sabresd_calibration_voltage(data);
#endif
		if (temp_last == 0) {
		...
		data->percent = sabresd_calibrate_battery_capability_percent(data);
...
static int sabresd_max8903_battery_get_property(struct power_supply *bat,
...
static int sabresd_max8903_get_property(struct power_supply *psy,
		enum power_supply_property psp,
		union power_supply_propval *val)
{
...
static int sabresd_max8903_get_usb_property(struct power_supply *usb,
		enum power_supply_property psp,
		union power_supply_propval *val)
{
...
static irqreturn_t sabresd_max8903_dcin(int irq, void *_data)
{
	...
	sabresd_max8903_charger_update_status(data);
	sabresd_max8903_battery_update_status(data);
	power_supply_changed(&data->psy);
	power_supply_changed(&data->bat);
	return IRQ_HANDLED;
}
static irqreturn_t sabresd_max8903_usbin(int irq, void *_data)
{
	struct max8903_data *data = _data;
	struct max8903_pdata *pdata = data->pdata;
	bool usb_in;
...
static irqreturn_t sabresd_max8903_fault(int irq, void *_data)
{
	struct max8903_data *data = _data;
	struct max8903_pdata *pdata = data->pdata;
	bool fault;
...
static irqreturn_t sabresd_max8903_chg(int irq, void *_data)
{
	struct max8903_data *data = _data;
	struct max8903_pdata *pdata = data->pdata;
	int chg_state;
	...
	//printk("%s %s %d chg_state=%d\n",__FILE__,__func__,__LINE__,chg_state); 
...
static void sabresd_max8903_battery_work(struct work_struct *work)
{
	...
	sabresd_max8903_charger_update_status(data);
	sabresd_max8903_battery_update_status(data);
	pr_debug("%s,battery voltage: %4d mV\n" ,__func__, data->voltage_uV);
	pr_debug("%s,charger online status: %d\n" , __func__,data->charger_online);
	pr_debug("%s,battery status : %d\n" , __func__,data->battery_status);
	pr_debug("%s,battery capacity percent: %3d\n" , __func__,data->percent);
	pr_debug("%s,data->usb_in: %x , data->ta_in: %x \n" , __func__,data->usb_in, data->ta_in);
	/* reschedule for the next time */
	schedule_delayed_work(&data->work, data->interval);
}

static ssize_t sabresd_max8903_voltage_offset_discharger_show(struct device *dev,
...
static ssize_t sabresd_max8903_voltage_offset_discharger_store(struct device *dev,
...
static ssize_t sabresd_max8903_voltage_offset_charger_show(struct device *dev,
			    struct device_attribute *attr, char *buf)
{
...
static ssize_t sabresd_max8903_voltage_offset_charger_store(struct device *dev,
			     struct device_attribute *attr, const char *buf,
			     size_t count)
{
...
static ssize_t sabresd_max8903_voltage_offset_usb_charger_show(struct device *dev,
			    struct device_attribute *attr, char *buf)
{
...
static ssize_t sabresd_max8903_voltage_offset_usb_charger_store(struct device *dev,
			     struct device_attribute *attr, const char *buf,
			     size_t count)
{
...
static struct device_attribute sabresd_max8903_discharger_dev_attr = {
	.attr = {
		 .name = "max8903_ctl_offset_discharger",
		 .mode = S_IRUSR | S_IWUSR,
		 },
	.show = sabresd_max8903_voltage_offset_discharger_show,
	.store = sabresd_max8903_voltage_offset_discharger_store,
};

static struct device_attribute sabresd_max8903_charger_dev_attr = {
	.attr = {
		 .name = "max8903_ctl_offset_charger",
		 .mode = S_IRUSR | S_IWUSR,
		 },
	.show = sabresd_max8903_voltage_offset_charger_show,
	.store = sabresd_max8903_voltage_offset_charger_store,
};

static struct device_attribute sabresd_max8903_usb_charger_dev_attr = {
	.attr = {
		 .name = "max8903_ctl_offset_usb_charger",
		 .mode = S_IRUSR | S_IWUSR,
		 },
	.show = sabresd_max8903_voltage_offset_usb_charger_show,
	.store = sabresd_max8903_voltage_offset_usb_charger_store,
};

static __devinit int sabresd_max8903_probe(struct platform_device *pdev)
{
	struct max8903_data *data;
	...
	//printk("%s %s %d cpu_is_mx6q()=%d ,cpu_is_mx6dl()=%d\n",__FILE__,__func__,__LINE__,cpu_is_mx6q(),cpu_is_mx6dl()); 
	cpu_type_flag = 0;
	if (cpu_is_mx6q())
		cpu_type_flag = 1;
	if (cpu_is_mx6dl())
	...
	if (pdata->dc_valid) {
		if (pdata->dok && gpio_is_valid(pdata->dok)) {
			...
			//printk("%s,%s,%d,ta_in=%d\n",__FILE__,__func__,__LINE__,ta_in);
	...
	data->psy.get_property = sabresd_max8903_get_property;
	data->psy.properties = sabresd_max8903_charger_props;
	data->psy.num_properties = ARRAY_SIZE(sabresd_max8903_charger_props);
	...
	data->usb.get_property = sabresd_max8903_get_usb_property;
	data->usb.properties = sabresd_max8903_charger_props;
	data->usb.num_properties = ARRAY_SIZE(sabresd_max8903_charger_props);
	...
	data->bat.type = POWER_SUPPLY_TYPE_USB_ACA;
	data->bat.properties = sabresd_max8903_battery_props;
	data->bat.num_properties = ARRAY_SIZE(sabresd_max8903_battery_props);
	data->bat.get_property = sabresd_max8903_battery_get_property;
	...
	INIT_DELAYED_WORK(&data->work, sabresd_max8903_battery_work);
	schedule_delayed_work(&data->work, data->interval);

       // printk("data->ta_in=%d,data->usb_in=%d,pdata->dc_valid=%d,pdata->usb_valid=%d \n",
	//		data->ta_in,
	//		data->usb_in,
	//		pdata->dc_valid,
	//		pdata->usb_valid);
	...
	ret = device_create_file(&pdev->dev, &sabresd_max8903_discharger_dev_attr);
	if (ret)
		dev_err(&pdev->dev, "create device file failed!\n");
	ret = device_create_file(&pdev->dev, &sabresd_max8903_charger_dev_attr);
	if (ret)
		dev_err(&pdev->dev, "create device file failed!\n");
	ret = device_create_file(&pdev->dev, &sabresd_max8903_usb_charger_dev_attr);
	if (ret)
		dev_err(&pdev->dev, "create device file failed!\n");
	if (cpu_type_flag == 1) {
...
static __devexit int sabresd_max8903_remove(struct platform_device *pdev)
{
...
static int sabresd_max8903_suspend(struct platform_device *pdev,
				  pm_message_t state)
{
	struct max8903_data *data = platform_get_drvdata(pdev);
	int irq;
...
static int sabresd_max8903_resume(struct platform_device *pdev)
{
	struct max8903_data *data = platform_get_drvdata(pdev);
	bool ta_in;
	bool usb_in;
...
static struct platform_driver sabresd_max8903_driver = {
	.probe	= sabresd_max8903_probe,
	.remove	= __devexit_p(sabresd_max8903_remove),
	.suspend = sabresd_max8903_suspend,
	.resume = sabresd_max8903_resume,
	.driver = {
		.name	= MAX8903_SABRESD_DRIVE_NAME,
		.owner	= THIS_MODULE,
	},
};

static int __init sabresd_max8903_init(void)
{
	return platform_driver_register(&sabresd_max8903_driver);
}
module_init(sabresd_max8903_init);

static void __exit sabresd_max8903_exit(void)
{
	platform_driver_unregister(&sabresd_max8903_driver);
}
module_exit(sabresd_max8903_exit);
