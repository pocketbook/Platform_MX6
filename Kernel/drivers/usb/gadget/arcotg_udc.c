...
void try_vbus_detection(struct fsl_udc *udc)
{
	struct fsl_usb2_platform_data *pdata;
	u32 irq_src;
	...
	if (irq_src & OTGSC_B_SESSION_VALID_IRQ_STS) {
		...
		fsl_writel(OTGSC_B_SESSION_VALID_IRQ_STS, &dr_regs->otgsc);
...
void imx_usb_charger_start_detect (void)
{
	unsigned long flags;
	struct fsl_usb2_platform_data *pdata;
	
	pdata = udc_controller->pdata;
	...
	if(udc_controller->transceiver->gadget == NULL) {
		if (fsl_readl(&dr_regs->otgsc)& OTGSC_B_SESSION_VALID ) {
			...
			dr_discharge_line(pdata, false);
			/* notify vbus is connected */
			imx_usb_vbus_connect(&udc_controller->charger);
			dr_wake_up_enable(udc_controller, true);
		} else {
			/* notify vbus is disconnected */
			imx_usb_vbus_disconnect(&udc_controller->charger);
			/* wait line to se0 */
			dr_discharge_line(pdata, true);			
		}		
	}
...
EXPORT_SYMBOL(imx_usb_charger_start_detect);
