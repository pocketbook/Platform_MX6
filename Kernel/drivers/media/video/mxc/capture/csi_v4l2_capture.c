...
#define MAXBUFFERSIZE (2592*1944*2)

static int video_nr = -1;
static cam_data *g_cam;
static int req_buf_number;
static int g_buffer_size=0, g_buffer_count=0;
...
static int pxp_process_update(cam_data *cam)
{
	...
	xrz_printk("Starting PxP Send Buffer\n");
	...
	xrz_printk("srect l: %d, t: %d, w: %d, h: %d; "
...
static void camera_callback(u32 mask, void *dev)
{
	...
	if (!list_empty(&cam->working_q)) {
...
#if 0
       {
            ...
            unsigned int data;
	   extern int savemode;
            unsigned int *ptr = done_frame->vaddress;
	    if(savemode == 0){
            for(ii=0; ii<(640*480*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }
	    }else if(savemode ==1){
	          for(ii=0; ii<(320*240*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	    }else if(savemode ==2){
	          for(ii=0; ii<(720*480*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	    }else if(savemode ==3){
	          for(ii=0; ii<(720*576*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	    }else if(savemode ==4){
	          for(ii=0; ii<(1280*720*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	    }else if(savemode ==5){
	          for(ii=0; ii<(1920*1080*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	    }else if(savemode == 6){
	          for(ii=0; ii<(2592*1944*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }
	    }else if(savemode ==7){
	          for(ii=0; ii<(176*144*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	    }else if(savemode ==8){
	          for(ii=0; ii<(1024*768*2)/4; ii++) {
                    data = ((*ptr)<<2) & 0xFCFCFCFC;
                      *(ptr++) = data;
	         }	    
	        }
...
static int csi_free_frame_buf(cam_data *cam)
{
	...
	printk("FREE_FRAME_BUF:\n");
	xrz_printk("MVC: In %s\n", __func__);

	if (cam->dummy_frame.vaddress != 0) {
		dma_free_coherent(0, PAGE_ALIGN(MAXBUFFERSIZE),
	...
	for (i = FRAME_NUM-1; i >= 0; --i) {
        ...
        g_buffer_size = g_buffer_count = 0;
	return 0;
}
...
static int csi_allocate_frame_buf(cam_data *cam, int count)
{
	...
	printk("ALLOCATE_FRAME_BUF: %d %d\n", count, cam->v2f.fmt.pix.sizeimage);
	xrz_printk("In MVC:%s- size=%d\n",
		 __func__, cam->v2f.fmt.pix.sizeimage);

	for (i = 0; i < count; i++) {
		if (cam->frame[i].vaddress == 0) {
			cam->frame[i].vaddress = dma_alloc_coherent(0,
							       PAGE_ALIGN(MAXBUFFERSIZE),
							       &cam->frame[i].paddress,
							       GFP_DMA | GFP_KERNEL);
		}
		if (cam->frame[i].vaddress == 0) {
		...
		cam->frame[i].buffer.length = PAGE_ALIGN(MAXBUFFERSIZE);
	...
	cam->dummy_frame.buffer.length = PAGE_ALIGN(MAXBUFFERSIZE);
...
{
	xrz_printk("In MVC:csi_v4l2_release_bufs\n");
	return 0;
}

static int csi_v4l2_prepare_bufs(cam_data *cam, struct v4l2_buffer *buf)
{
	xrz_printk("In MVC:csi_v4l2_prepare_bufs\n");
...
static int csi_v4l2_g_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		xrz_printk("   type is V4L2_BUF_TYPE_VIDEO_CAPTURE\n");
		f->fmt.pix = cam->v2f.fmt.pix;
		break;
	case V4L2_BUF_TYPE_VIDEO_OVERLAY:
		xrz_printk("   type is V4L2_BUF_TYPE_VIDEO_OVERLAY\n");
		f->fmt.win = cam->win;
		break;
	default:
		xrz_printk("   type is invalid\n");
		retval = -EINVAL;
	}

	xrz_printk("End of %s: v2f pix widthxheight %d x %d\n",
...
static int csi_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	xrz_printk("In MVC: %s,f->type=%d \n", __func__,f->type);

	switch (f->type) {
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		xrz_printk("   type=V4L2_BUF_TYPE_VIDEO_CAPTURE\n");
		...
		if ((f->fmt.pix.width > cam->crop_bounds.width)
			 ...
			 * camera can change. */
			xrz_printk("csi_v4l2_s_fmt size changed\n");
		}
		if (cam->rotation % 180) {
		...
		xrz_printk("In MVC: %s,f->fmt.pix.pixelformat=%d \n", __func__,f->fmt.pix.pixelformat);
		switch (f->fmt.pix.pixelformat) {
		case V4L2_PIX_FMT_RGB565:
			xrz_printk("type:V4L2_PIX_FMT_RGB565 \n");
		...
		case V4L2_PIX_FMT_UYVY:
			xrz_printk("type:V4L2_PIX_FMT_UYVY \n");
			size = f->fmt.pix.width * f->fmt.pix.height * 2;
		...
		case V4L2_PIX_FMT_YUYV:
			xrz_printk("type:V4L2_PIX_FMT_YUYV \n");
			size = f->fmt.pix.width * f->fmt.pix.height * 2;
			csi_init_format(f->fmt.pix.pixelformat);
			csi_set_16bit_imagpara(f->fmt.pix.width,
					       f->fmt.pix.height);
		...
		case V4L2_PIX_FMT_YUV420:
			xrz_printk("type:V4L2_PIX_FMT_YUV420 \n");
		...
		break;
	case V4L2_BUF_TYPE_VIDEO_OVERLAY:
		xrz_printk("   type=V4L2_BUF_TYPE_VIDEO_OVERLAY\n");
		cam->win = f->fmt.win;
...
static int csi_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	xrz_printk("   Current capabilities are %x\n",
		 currentparm.parm.capture.capability);
	xrz_printk("   Current capturemode is %d  change to %d\n",
		 currentparm.parm.capture.capturemode,
		 parm->parm.capture.capturemode);
	xrz_printk("   Current framerate is %d  change to %d\n",
	...
	xrz_printk("   g_fmt_cap returns widthxheight of input as %d x %d\n",
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_OVERLAY:{
			int *on = arg;
			xrz_printk("   case VIDIOC_OVERLAY\n");
			if (*on) {
				cam->overlay_on = true;
	...
	case VIDIOC_QUERYCAP:{
			...
			xrz_printk("   case VIDIOC_QUERYCAP\n");
	...
	case VIDIOC_REQBUFS: {
		...
		xrz_printk("   case VIDIOC_REQBUFS\n");
	...
	case VIDIOC_QUERYBUF: {
		...
		int index = buf->index;
		xrz_printk("   case VIDIOC_QUERYBUF\n");
	...
	case VIDIOC_STREAMON: {
		xrz_printk("   case VIDIOC_STREAMON\n");
	...
	case VIDIOC_STREAMOFF: {
		xrz_printk("   case VIDIOC_STREAMOFF\n");
	...
	case VIDIOC_S_CTRL:
	{
		...
		if (i >= ARRAY_SIZE(pxp_controls))
			retval = vidioc_int_s_ctrl(cam->sensor, (struct v4l2_control *)vc);
		break;
	...
	case VIDIOC_G_CTRL:
	{
		...
		if (i >= ARRAY_SIZE(pxp_controls))
			retval = vidioc_int_g_ctrl(cam->sensor, (struct v4l2_control *)vc);
		break;
	}
...
static int __devinit csi_v4l2_probe(struct platform_device *pdev)
{
	...
	xrz_printk("   Video device registered: %s #%d\n",
...
static int csi_v4l2_master_attach(struct v4l2_int_device *slave)
{
	...
	xrz_printk("In MVC: %s\n", __func__);
	xrz_printk("   slave.name = %s\n", slave->name);
	xrz_printk("   master.name = %s\n", slave->u.slave->master->name);
