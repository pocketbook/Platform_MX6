...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
  ...
  //x=GTP_MAX_WIDTH-x;/*for myandroidr13.5-alpha */
    ...
    GTP_DEBUG("%s,ts->abs_x_max=%d,ts->abs_y_max=%d,ID:%d, X:%d, Y:%d, W:%d",__func__, 
    ts->abs_x_max,ts->abs_y_max,id, x, y, w);
}
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
  ...
  //	testx = report_max_x;
  //	testy= report_max_y;
   //printk("%s,report_max_x=%d,report_max_y=%d \n",__func__,report_max_x,report_max_y);
#if GTP_CHANGE_X2Y
