...
static bool bNeedResumeGTP= false;
#endif
...
/*******************************************************
Function:
	Enable IRQ Wakeup the Device Function.
...
********************************************************/
#if  defined(CONFIG_XRZ_EBOOKREADING_MODE)
void gtp_irp_enable_wakeup(struct goodix_ts_data *ts)
{
    unsigned long irqflags;
...
#if  defined(CONFIG_XRZ_EBOOKREADING_MODE)
	if((iXrzSuspendMode == 0) && bNeedResumeGTP){
		bNeedResumeGTP = false;
