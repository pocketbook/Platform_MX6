...
int aic3xxx_device_init(struct aic3xxx *aic3xxx)
{
	...
	if (aic3xxx->pdata.gpio_spken) {
		ret = gpio_request(aic3xxx->pdata.gpio_spken, "aic3xxx-spken-pin");
		if (ret != 0) {
			dev_err(aic3xxx->dev, "not able to acquire gpio\n");
			goto err_return;
		}
		gpio_direction_output(aic3xxx->pdata.gpio_spken, 0);
		gpio_free(aic3xxx->pdata.gpio_spken);
	}
...
//{
///i2c write 
 // aic3xxx_reg_write(aic3xxx,AIC3256_PAGE_SELECT,0x21);
//  msleep(100);
  ///I2C read
 //int value= aic3xxx_reg_read(aic3xxx,AIC3256_LEFT_AGC_REG3);
  //printk("%s,%s,value=%d \n",__FILE__,__func__,value);

  //aic3xxx_reg_write(aic3xxx,AIC3256_PAGE_SELECT,0);
//}
//#endif
