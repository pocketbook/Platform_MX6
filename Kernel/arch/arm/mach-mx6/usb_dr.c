...
extern void imx_usb_charger_start_detect(void);
#endif
...
static void usbotg_wakeup_event_clear(void)
{
	...
	UOG_USBSTS |= 0x5FF; // clear USBSTS interrupt status
	UOG_OTGSC |= 0x007F0000; // clear OTGSC interrupt status
}
...
static void _device_wakeup_enable(struct fsl_usb2_platform_data *pdata, bool enable)
{
	...
	if (enable) {
#ifdef CONFIG_IMX_USB_CHARGER
		if(!pdata->port_enables) {
			UOG_OTGSC |= (1<<27);//OTGSC_B_SESSION_VALID_IRQ_EN
		}
#endif
		...
		if (!pdata->port_enables)
			return;
		UOG_OTGSC |= (1<<19);//OTGSC_B_SESSION_VALID_IRQ_STS
#endif
...
static void device_wakeup_handler(struct fsl_usb2_platform_data *pdata)
{
     ...
     * cycles of the standby clock(32k Hz) , that is 0.094 ms*/
    udelay(100);
	imx_usb_charger_start_detect();
    _device_wakeup_enable(pdata, true);
#else
	_device_phy_lowpower_suspend(pdata, false);
