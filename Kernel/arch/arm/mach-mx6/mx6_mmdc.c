...
void (*mx6l_ddr3_change_freq)(u32 freq, void *ddr_settings, bool dll_mode, void* iomux_offsets, int low_bus_freq_mode) = NULL;
...
int update_ddr_freq(int ddr_rate)
{
	...
	*((char *)(&cpus_in_wfe) + (u8)me) = 0xff;
	...
	if (cpu_is_mx6sl()){
		mx6l_ddr3_change_freq(ddr_rate, iram_ddr_settings, dll_off, iram_iomux_settings, low_bus_freq_mode);
	} else {
...
int init_mmdc_settings(void)
{
	...
	if (!cpu_is_mx6sl()) {
	  for_each_online_cpu(cpu) {
		...
		if (err) {
				printk(KERN_ERR "%s,%s,MMDC: unable to set irq affinity irq=%ld,\n", __FILE__,__func__,irq_used[cpu]);
			return err;
		}
	  }
	}
