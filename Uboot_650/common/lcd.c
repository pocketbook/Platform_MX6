...
static int lcd_init (void *lcdbase)
{
	...
	debug ("%s,%s,[LCD] Initializing LCD frambuffer at %p\n", __FILE__,__func__,lcdbase);
...
int lcd_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	if (!((bmp->header.signature[0]=='B') &&
		...
		for (i=0; i<100; i++) {
			memset(fb,     0x00, 64);
			memset(fb+64,  0x55, 64);
			memset(fb+128, 0xaa, 64);
			memset(fb+192, 0xff, 64);
			fb += lcd_line_length;
		}
		return 0;
	}
	...
	if (bmp_bpix != 8) 
	{
		printf ("Error: BMP has %d bit/pixel\n",
			le16_to_cpu(bmp->header.bit_count));
		return 1;
	}

	printf ("%s,%s,Display-bmp: %d x %d  with %d bpp\n",__FILE__,__func__,
		(int)width, (int)height, (int)bmp_bpix);

	for (i=0; i<256; ++i) {
		bmp_color_table_entry_t cte = bmp->color_table[i];
		lcd_setcolreg(i, cte.red, cte.green, cte.blue);
		palette[i] = (cte.red * 77 + cte.green * 151 + cte.blue * 28) >> 8;
	}
...
static void *lcd_logo (void)
{
	...
	if (do_splash && (s = getenv("splashimage")) != NULL) {
		...
		printf("bmp->header.signature[0]=%s,\n,bmp->header.signature[1]=%s,\n",
