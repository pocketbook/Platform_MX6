...
extern void wait_splash_screen_update();
...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	case IH_COMP_NONE:
		if (load == blob_start) {
			...
			printf ("   Loading %s (%x->%x) ... ", type_name, image_start, load);
	...
	wait_splash_screen_update();
	epdc_power_off();

	if ((load < blob_end) && (*load_end > blob_start) && load != image_start) {
